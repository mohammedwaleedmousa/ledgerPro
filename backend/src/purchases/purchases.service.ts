...
type PurchaseOrderRow = { id: string; supplier_id: string; purchase_number: string; issue_date: string; expected_date: string; status: 'draft' | 'ordered' | 'received' | 'cancelled'; notes: string; subtotal: string | number; tax_rate: string | number; tax_amount: string | number; total: string | number; created_at: string };
type PurchaseItemRow = { purchase_order_id: string; id: string; product_id: string; product_name: string; quantity: number; unit_cost: string | number; total: string | number };
...
export class PurchasesService {
  ...
  async list(user: AuthenticatedUser, limit = 100) {
    ...
    const params = new URLSearchParams({ company_id: `eq.${user.companyId}`, select: 'id,supplier_id,purchase_number,issue_date,expected_date,status,notes,subtotal,tax_rate,tax_amount,total,created_at', order: 'issue_date.desc,id.desc', limit: String(safeLimit) });
    ...
    if (supplierIds.length) {
      ...
      if (supplierResponse.ok) ((await supplierResponse.json()) as Array<{ id: string; name: string }>).forEach((supplier) => supplierNames.set(supplier.id, supplier.name));
    }
    const orderIds = rows.map((row) => row.id);
    const itemsByOrder = new Map<string, PurchaseItemRow[]>();
    if (orderIds.length) {
      const itemParams = new URLSearchParams({ company_id: `eq.${user.companyId}`, purchase_order_id: `in.(${orderIds.join(',')})`, select: 'id,purchase_order_id,product_id,product_name,quantity,unit_cost,total', order: 'created_at.asc' });
      const itemResponse = await fetch(`${this.supabaseUrl}/rest/v1/purchase_order_items?${itemParams.toString()}`, { headers: this.headers() });
      if (itemResponse.ok) ((await itemResponse.json()) as PurchaseItemRow[]).forEach((item) => itemsByOrder.set(item.purchase_order_id, [...(itemsByOrder.get(item.purchase_order_id) ?? []), item]));
    }
    return rows.map((row) => ({ id: row.id, number: row.purchase_number, supplierId: row.supplier_id, supplierName: supplierNames.get(row.supplier_id) ?? 'مورد', issueDate: row.issue_date, expectedDate: row.expected_date, status: row.status, notes: row.notes, items: (itemsByOrder.get(row.id) ?? []).map((item) => ({ id: item.id, productId: item.product_id, productName: item.product_name, quantity: item.quantity, unitCost: Number(item.unit_cost), total: Number(item.total) })), subtotal: Number(row.subtotal), taxRate: Number(row.tax_rate), taxAmount: Number(row.tax_amount), total: Number(row.total), createdAt: row.created_at }));
  }

  async create(user: AuthenticatedUser, input: PurchaseOrderWriteInput, requestKey: string) {
    this.assertConfigured();
    ...
    const response = await fetch(`${this.supabaseUrl}/rest/v1/rpc/create_purchase_order_idempotent`, {
      method: 'POST', headers: this.headers({ 'Content-Type': 'application/json' }),
      body: JSON.stringify({ p_actor_id: user.id, p_supplier_id: input.supplierId, p_issue_date: input.issueDate, p_expected_date: input.expectedDate, p_status: input.status, p_tax_rate: input.taxRate, p_notes: input.notes?.trim() ?? '', p_items: input.items.map((item) => ({ product_id: item.productId, quantity: item.quantity, unit_cost: item.unitCost })), p_request_key: requestKey }),
    });
    if (!response.ok) throw new BadRequestException(await this.errorMessage(response, 'Unable to create purchase order.'));
    return response.json();
  }

  async receive(user: AuthenticatedUser, purchaseOrderId: string, requestKey: string) {
    this.assertConfigured();
    const response = await fetch(`${this.supabaseUrl}/rest/v1/rpc/receive_purchase_order_idempotent`, { method: 'POST', headers: this.headers({ 'Content-Type': 'application/json' }), body: JSON.stringify({ p_actor_id: user.id, p_purchase_order_id: purchaseOrderId, p_request_key: requestKey }) });
  ...
  async markOrdered(user: AuthenticatedUser, purchaseOrderId: string) {
    ...
    const response = await fetch(`${this.supabaseUrl}/rest/v1/purchase_orders?${params.toString()}`, { method: 'PATCH', headers: this.headers({ 'Content-Type': 'application/json', Prefer: 'return=representation' }), body: JSON.stringify({ status: 'ordered' }) });
