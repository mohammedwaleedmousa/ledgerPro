...
type MovementRow = {
  id: string;
  product_id: string;
  movement_type: 'opening' | 'sale' | 'adjustment' | 'purchase' | 'return' | 'transfer_in' | 'transfer_out';
...
export class InventoryService {
  ...
  async movements(user: AuthenticatedUser, limit = 100) {
    ...
    const params = new URLSearchParams({
      ...
      select: 'id,product_id,movement_type,quantity_delta,balance_after,reference,created_at',
    ...
    if (productIds.length) {
      ...
      if (productsResponse.ok) {
        ...
        products.forEach((product) => names.set(product.id, product.name));
      }
    }
    ...
    return rows.map((row) => ({
      ...
      productName: names.get(row.product_id) ?? 'منتج',
      type: row.movement_type === 'transfer_in' || row.movement_type === 'transfer_out' ? 'adjustment' : row.movement_type,
  ...
  async adjust(user: AuthenticatedUser, input: StockAdjustmentInput): Promise<StockAdjustmentResult> {
    this.assertConfigured();
    ...
    const response = await fetch(`${this.supabaseUrl}/rest/v1/rpc/adjust_stock`, {
      method: 'POST',
