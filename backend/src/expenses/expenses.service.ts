...
type ExpenseRow = { id: string; supplier_id: string | null; category: string; description: string; amount: string | number; expense_date: string; status: 'paid' | 'pending'; created_at: string };
...
export class ExpensesService {
  ...
  async list(user: AuthenticatedUser, limit = 100) {
    ...
    const params = new URLSearchParams({ company_id: `eq.${user.companyId}`, select: 'id,supplier_id,category,description,amount,expense_date,status,created_at', order: 'expense_date.desc,id.desc', limit: String(Math.max(1, Math.min(Number(limit) || 100, 200))) });
  ...
  async post(user: AuthenticatedUser, input: { category: string; description: string; amount: number; date: string; status: 'paid' | 'pending'; supplierId?: string; paymentMethod?: 'cash' | 'bank' | 'card' }, requestKey: string) {
    this.assertConfigured();
    ...
    const response = await fetch(`${this.supabaseUrl}/rest/v1/rpc/post_expense_idempotent`, {
      method: 'POST', headers: this.headers({ 'Content-Type': 'application/json' }),
      body: JSON.stringify({ p_actor_id: user.id, p_category: input.category.trim(), p_description: input.description.trim(), p_amount: input.amount, p_expense_date: input.date, p_status: input.status, p_supplier_id: input.supplierId || null, p_payment_method: input.status === 'paid' ? input.paymentMethod : null, p_request_key: requestKey }),
    });
