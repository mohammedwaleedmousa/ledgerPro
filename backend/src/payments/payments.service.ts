...
export class PaymentsService {
  ...
  async list(user: AuthenticatedUser, limit = 100) {
    ...
    const params = new URLSearchParams({
      ...
      select: 'id,payment_number,direction,party_type,customer_id,supplier_id,party_name,payment_date,method,amount,reference,notes,created_at',
  ...
  async post(user: AuthenticatedUser, input: PaymentWriteInput): Promise<PaymentMutationResult> {
    this.assertConfigured();
    ...
    const response = await fetch(`${this.supabaseUrl}/rest/v1/rpc/post_payment`, {
      method: 'POST',
