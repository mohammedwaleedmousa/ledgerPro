...
export class ErpService {
  ...
  async bootstrap(user: AuthenticatedUser) {
    ...
    const settingsParams = new URLSearchParams({
      company_id: companyFilter,
      select: 'currency,tax_rate,invoice_prefix,quotation_prefix,purchase_prefix,fiscal_year_start',
      limit: '1',
    });
    ...
    return {
      ...
      settings: {
        ...
        taxRate: asNumber(settings.tax_rate),
        invoicePrefix: settings.invoice_prefix,
        quotationPrefix: settings.quotation_prefix,
  ...
  async products(user: AuthenticatedUser, query: Record<string, unknown>): Promise<PageResult<unknown>> {
    ...
    const params = new URLSearchParams({
      ...
      select: 'id,name,sku,category_id,cost,price,stock,low_stock_threshold,description,is_active,created_at',
  ...
  async customers(user: AuthenticatedUser, query: Record<string, unknown>): Promise<PageResult<unknown>> {
    ...
    const params = new URLSearchParams({
      company_id: `eq.${user.companyId}`,
      select: 'id,name,email,phone,tax_number,address,balance,status,notes,created_at',
      order: 'created_at.desc,id.desc',
      limit: String(limit),
      offset: String(offset),
    });
  ...
  async invoices(user: AuthenticatedUser, query: Record<string, unknown>): Promise<PageResult<unknown>> {
    ...
    const params = new URLSearchParams({
      company_id: `eq.${user.companyId}`,
      select: 'id,customer_id,invoice_number,issue_date,payment_method,status,notes,subtotal,tax_rate,tax_amount,total,created_at',
    ...
    return {
      ...
      items: result.data.map((row) => ({
        ...
        customerName: customerNames.get(row.customer_id) ?? 'عميل',
  ...
  async invoiceDetails(user: AuthenticatedUser, invoiceId: string) {
    ...
    const itemsParams = new URLSearchParams({
      ...
      select: 'id,invoice_id,product_id,product_name,quantity,unit_price,unit_cost,total',
